...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for ix in range(len(self.info['images'])):
            ...
            # # To eval on 2014 val set
            # if img['split'] != 'train':
            #     self.split_ix['test'].append(ix)


            # To train on the whole training set
            # self.split_ix['train'].append(ix)
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            if self.use_box:
                ...
                # x1,y1,x2,y2,area = np.hsplit(box_feat, 5)
                # # h,w = self.info['images'][ix]['height'], self.info['images'][ix]['width']
                # # box_feat = np.hstack((x1/w, y1/h, (x2-x1)/w, (y2-y1)/h, (x2-x1)*(y2-y1)/(w*h))) # question? x2-x1+1??
                # # if self.norm_box_feat:
                # #     box_feat = box_feat / np.linalg.norm(box_feat, 2, 1, keepdims=True)
                # h, w = y2-y1, x2-x1
                # box_feat = np.hstack((x1,y1,h,w, area))
...
class BlobFetcher():
    ...
    def reset(self):
        ...
        self.split_loader = iter(data.DataLoader(dataset=self.dataloader,
                                            ...
                                            shuffle=False,
                                            pin_memory=True,
                                            num_workers=10, # 4 is usually enough
