...
class MultiHeadedDotAttentionMemory(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, norm_q=0, dropout_glu=0.3):
        super(MultiHeadedDotAttentionMemory, self).__init__()
...
class MultiHeadModelNew(AttModel):
    def __init__(self, opt):
        super(MultiHeadModelNew, self).__init__(opt)
        self.num_layers = 1
    ...
    # on machine12:
    # CUDA_VISIBLE_DEVICES=3 sh train_mh_new.sh
    # CUDA_VISIBLE_DEVICES=3 nohup sh train_mh_new.sh > train_trans_both_glu_drop3.log &
