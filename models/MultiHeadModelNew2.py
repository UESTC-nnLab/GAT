...
class MultiHeadCore(nn.Module):
    def __init__(self, opt):
        ...
        # self.h2q = nn.Linear(opt.rnn_size, self.d_model)
        # residual
        ...
        attn = MultiHeadedDotAttention(opt.num_heads, self.d_model, project_k_v=1,
                                       scale=self.multi_head_scale, norm_q=0)
...
class MultiHeadModelNew2(AttModel):
    def __init__(self, opt):
        super(MultiHeadModelNew2, self).__init__(opt)
        self.num_layers = 1
    ...
    # on machine12:
    # CUDA_VISIBLE_DEVICES=0 sh train_mh_new2.sh
    # CUDA_VISIBLE_DEVICES=0 nohup sh train_mh_new2.sh > train_trans_both_glu.log &
