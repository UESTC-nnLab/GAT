...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, norm_q=0, dropout_glu=0.3):
        ...
        self.linears = clones(nn.Linear(d_model, d_model * scale), 3 + 2 * project_k_v)
    ...
    def forward(self, memory, query, key, value, mask=None):
        if mask is not None:
            if len(mask.size()) == 2:
        ...
        if self.project_k_v == 0:
            ...
            key_m = self.linears[1](memory).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value_m = self.linears[2](memory).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            key_ = key.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value_ = value.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        else:
            query_, key_m, value_m, key_, value_ = \
                [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
                 for l, x in zip(self.linears, (query, memory, memory, key, value))]

        # visual sentinel
        # Apply attention on all the projected vectors in batch.
        x, self.attn = attention(query_, torch.cat([key_, key_m], 2), torch.cat([value_, value_m], 2), mask=mask,
                                 dropout=self.dropout)
...
class MultiHeadCore(nn.Module):
    def __init__(self, opt):
        ...
        self.lang_lstm = nn.LSTMCell(opt.rnn_size + self.d_model + opt.input_encoding_size,
        ...
        # self.h2att = nn.Linear(opt.rnn_size, self.d_model)
        # residual
        # self.h2h_bar = nn.Linear(opt.rnn_size + opt.input_encoding_size, opt.rnn_size)

        # output, concat h1 and h2
        # self.fc_out = nn.Linear(opt.rnn_size * 2, opt.rnn_size)

        # memory gate
        self.fc_gate = nn.Linear(opt.rnn_size + opt.input_encoding_size, opt.rnn_size)
        self.fc_mem = nn.Linear(opt.rnn_size, self.d_model)
        ...
        # self.encoder = Encoder(EncoderLayer(self.d_model, attn, c(ff), 0.1), 3)

        # self.decoder = Decoder(DecoderLayer(self.d_model, attn, attn, c(ff), 0.1), 1)
        # self.decoder = Decoder(DecoderLayer(self.d_model, attn, c(ff), 0.1), 1)  # no FF
        self.vis_mh_attention = MultiHeadedDotAttention(opt.num_heads, self.d_model, project_k_v=1,
                                                        scale=opt.multi_head_scale, norm_q=1)

        # self.box_embed = nn.Sequential(nn.Linear(4, self.d_model),
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        # vis_refined = self.encoder(p_att_feats, att_masks)

        # memory, visual sentinel
        gate = self.fc_gate(torch.cat([h_att, xt], -1))
        memory = torch.sigmoid(gate) * torch.tanh(c_att)
        memory = self.fc_mem(memory)  # project to d_model dim

        # MultiHeadedDotAttention(query, key, value, mask)
        q = self.h2q(torch.cat([h_att, xt], -1))
        # q = self.h2q(h_att)
        vis_att = self.vis_mh_attention(memory, q, vis_refined, vis_refined, None)  # [batch_size, 1, 512]

        lang_lstm_input = torch.cat([vis_att, h_att, xt], 1)
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], state[1][1]))

        state = (torch.stack([h_att, h_lang]), torch.stack([c_att, c_lang]))
        # output = self.fc_out(F.dropout(torch.cat([h_lang, h_att], -1), self.drop_prob_lm, self.training))
...
class MultiHeadModel(AttModel):
    ...
    # on machine12:
    # CUDA_VISIBLE_DEVICES=1 sh train_mh.sh
    # CUDA_VISIBLE_DEVICES=1 nohup sh train_mh.sh > train_mh_box_embed_concatQK_glu_add_m.log &
