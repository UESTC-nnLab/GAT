...
def box_attention(query_g, key_g, query_a, key_a, value_a, mask=None, dropout=None):
    ...
    gate = torch.sigmoid(scores_g)
    # Appearance
    scores_a = torch.matmul(query_a, key_a.transpose(-2, -1)) \
               / np.sqrt(d_k)
    # element-wise product
    scores = scores_a * gate

    if mask is not None:
        scores = scores_a.masked_fill(mask == 0, -1e9)
...
class BoxMultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        self.linears_G = clones(nn.Linear(64, d_model), 2)
    ...
    def forward(self, query_g, key_g, query_a, key_a, value_a, mask=None):
        ...
        x, self.attn = box_attention(query_g, key_g, query_a, key_a, value_a, mask=mask,
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        "Implements Figure 2. 输入的q，k，v是形状 [batch, L, d_model]。"
        if mask is not None:
            # Same mask applied to all h heads.
            mask = mask.unsqueeze(1)
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        # This means the original meaning in the embeddings won't be lost when added together.
...
class TransformerModel(AttModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, N=6,
        ...
        box_embed = nn.Sequential(nn.Linear(4, 64),
                                  nn.ReLU(),
                                  nn.Dropout(0.1))
    ...
    def __init__(self, opt):
        ...
        self.att_embed = nn.Sequential(*(
                ...
                (nn.Linear(2048, self.input_encoding_size),
                 nn.ReLU(),
    ...
    # CUDA_VISIBLE_DEVICES=0 nohup sh train_mh.sh > train_mh.log &
    # CUDA_VISIBLE_DEVICES=0 sh train_transformer.sh
