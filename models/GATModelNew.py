...
class BoxMultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1, use_glu=1, dropout_glu=0.5):
        ...
        if self.use_glu:
            self.fc_gate = nn.Linear(d_model * 3, d_model)
            self.fc_info = nn.Linear(d_model * 3, d_model)
            # self.glu_layer = nn.Sequential(nn.Linear(3 * d_model, 2 * d_model), nn.GLU())
    ...
    def forward(self, query_g, key_g, query_a, key_a, value_a, mask=None):
        ...
        if self.use_glu:
            # Apply GLU
            gate = self.fc_gate(self.dropout_glu(torch.cat([query_a, query_g, x], -1)))
            info = self.fc_info(self.dropout_glu(torch.cat([query_a, query_g, x], -1)))
            x = torch.sigmoid(gate) * info
            # x = self.glu_layer(self.dropout_glu(torch.cat([x, query_a, query_g], -1)))

        return self.linears_out(x)


class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, project_k_v=1, norm_q=0, use_glu=1, dropout_glu=0.5):
        ...
        self.linears = clones(nn.Linear(d_model, d_model), 1 + 2 * project_k_v)
        ...
        if self.use_glu:
            ...
            self.fc_info = nn.Linear(d_model * 2, d_model)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if self.use_glu:
            # Apply GLU
            gate = self.fc_gate(self.dropout_glu(torch.cat([x, query], -1)))
            info = self.fc_info(self.dropout_glu(torch.cat([x, query], -1)))
            x = torch.sigmoid(gate) * info
...
class GATCore(nn.Module):
    def __init__(self, opt):
        ...
        c = copy.deepcopy
        box_attn = BoxMultiHeadedAttention(opt.num_heads, self.d_model, use_glu=opt.use_glu_enc)
        attn = MultiHeadedDotAttention(opt.num_heads, self.d_model, project_k_v=1,
                                       norm_q=0, use_glu=opt.use_glu_dec)
        ff = PositionwiseFeedForward(self.d_model, 2048, 0.1)
        # Geometrical self-attention refiner
        self.encoder = BoxEncoder(BoxEncoderLayer(self.d_model, box_attn, c(ff), 0.1), opt.N_model)
        # self.encoder = BoxEncoder(BoxEncoderLayer(self.d_model, box_attn, None, 0.1), 3)  # no ff

        # Self-attention module
        self.decoder = MyDecoder(MyDecoderLayer(self.d_model, attn, c(ff), 0.1), opt.N_model)
        # self.decoder = MyDecoder(MyDecoderLayer(self.d_model, attn, None, 0.1), 3)  # no ff
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        # [batch_size, 36, 512]
        vis_refined = self.encoder(p_att_feats, geo_embd, att_masks)

        # MultiHeadedDotAttention(query, key, value, mask)
        # q = self.h2q(torch.cat([h_att, xt], -1))
        q = self.norm(h_att)
        vis_att = self.decoder(q, vis_refined, None)  # [batch_size, 1, 512]
...
class GATModelNew(AttModel):
    def __init__(self, opt):
        super(GATModelNew, self).__init__(opt)
        self.num_layers = 1
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        # p_att_feats = self.att_embed(att_feats[:, :, :20  48])
        p_att_feats = self.att_embed(torch.cat([att_feats[:, :, :2048], att_feats[:, :, -5:]], -1))

        return fc_feats, att_feats, p_att_feats, att_masks

    # on machine12:
    # CUDA_VISIBLE_DEVICES=1 sh train_GAT0.sh

    # CUDA_VISIBLE_DEVICES=2 nohup sh train_GAT3.sh > train_GAT_enc_glu_new_beam5.log &

    # CUDA_VISIBLE_DEVICES=3 nohup sh train_GAT2.sh > train_GAT_both_glu_new_drop5.log &

    # CUDA_VISIBLE_DEVICES=0 nohup sh train_GAT1.sh > train_GAT_both_glu_new_beam5_4l.log &

    # CUDA_VISIBLE_DEVICES=0,1,2 nohup sh train_GAT1.sh > train_GAT_both_glu_new_beam5_4l.log &

    # on machine:
    # nohup sh train_GAT1.sh > train_GAT_both_glu_new_beam5.log &
